    ...
    ngOnInit() {
        this.cs.getTest().subscribe(books => {
            this.books = books.map(res => {
                return Object.assign({ id: res.payload.doc.id }, res.payload.doc.data());
            });
            console.log(this.books);
        });
        this.cs.getCart3().subscribe(cart2 => {
            this.books2 = cart2.map(res2 => {
                return Object.assign({ id: res2.payload.doc.id }, res2.payload.doc.data());
            });
        });
        this.cs.getCart2().subscribe(birth => {
            this.birth = birth.map(ress => {
                return Object.assign({ id: ress.payload.doc.id }, ress.payload.doc.data());
            });
        });
        this.cs.getCart4().subscribe(wedd => {
            this.wedd = wedd.map(resss => {
                return Object.assign({ id: resss.payload.doc.id }, resss.payload.doc.data());
            });
        });
    }
};
...
let BirthdayComponent = class BirthdayComponent {
    ...
    combook(name, date, numberr, size) {
        const data3 = {
            name1: name,
            dated2: date,
...
let BookFirstRoomComponent = class BookFirstRoomComponent {
    // books: Book[] = [];
    // tslint:disable-next-line: no-inferrable-types
    // add: number = -1;
    constructor() { }
...
let BookComponent = class BookComponent {
    constructor(route, cs, bs, calendar, as) {
        this.route = route;
        this.cs = cs;
        this.bs = bs;
        this.as = as;
    ...
    combook(name, date1, date2, type, young, child, room) {
        if (!(this.as.userId)) {
    ...
    combook2(name, date1, date2, type, young, child, room) {
        if (!(this.as.userId)) {
            this.route.navigate(['/logIn']);
        }
        // tslint:disable-next-line: one-line
...
let LogInComponent = class LogInComponent {
    ...
    constructor(as, route, af, authService, afAuth) {
        this.as = as;
        this.route = route;
        this.af = af;
    ...
    lognIn(form) {
        ...
        this.as.logIn(data.email, data.password)
            .then(result => { this.route.navigate(['/']); })
...
let SecondRoomComponent = class SecondRoomComponent {
    constructor() { }
    ngOnInit() {
    }
};
...
let SignInComponent = class SignInComponent {
    ...
    signIn(form) {
        ...
        this.as.signIn(data.email, data.password)
            ...
            this.errorMassage = '';
            // tslint:disable-next-line: align
            this.us.addNewUser(result.user.uid, data.name, data.phone, data.email, data.password, data.city).then(() => {
                this.route.navigate(['/']);
            });
        })
...
let WeddingComponent = class WeddingComponent {
    ...
    combook(name, date, time, numberr) {
        const data = {
            name1: name,
            date1: date,
            time1: time,
...
WeddingComponent.ctorParameters = () => [
    { type: src_app_services_cart_service__WEBPACK_IMPORTED_MODULE_2__["CartService"] }
];
